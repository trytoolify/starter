...
function TicketDetails({ticketId}: { ticketId: number }) {
    ...
    const fetcher: Fetcher<Ticket, string> = (id) => fetch(`https://app.trycreo.com/mock-api/support-tickets/${id}`).then((res) => res.json());
    ...
    return (
        <div className="py-6">
            ...
            <hr className="my-2"/>
            <p className="text-gray-500">Created By: {data?.customerName} ({data?.email})</p>
            <p className="text-gray-500">Priority: {data?.priority}</p>
            <p className="text-gray-500">Status: <Badge
                className={getStatusClassName(data?.status!)}>{data?.status}</Badge></p>
